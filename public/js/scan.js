    ...
    function updateAnalyzeButton() {
        if (isValidEAN(eanInput.value) && croppedImages.length > 0) {
    ...
    analyzeAllBtn.addEventListener('click', async () => {
        ...
        try {
            // Retrieve the EAN input value
            const eanInput = document.getElementById('eanInput').value.trim();
    
            // Validate the EAN input
            if (!isValidEAN(eanInput)) {
                throw new Error('Invalid EAN Code. Please provide a valid 13-digit EAN.');
            }
            ...
            const formData = new FormData();
            formData.append('ean', eanInput); // Add EAN value to FormData
            ...
            if (!detectResponse.ok) {
                throw new Error('No EAN Code Provided or Failed to detect text from images.');
            }
            ...
            const analyzeResponse = await fetch('/api/analyze-text', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ detectedText: detectedTexts, ean: eanInput }), // Include EAN in JSON payload
            });
                ...
                <div class="bg-red-50 p-4">
                    <div class="flex items-center justify-between space-x-2">
                        ...
                        <div class="flex items-center text-red-700">
                           <i data-feather="alert-circle mr-2"></i>
                           Invalid
                        </div>
